...
actual class LocalAzoraProjectRepository(
    ...
    override suspend fun createProject(project: AzoraProjectModel): Res<AzoraProjectModel, DataError.Local> {
        return try {
            ...
            // Scaffold template-specific source. The builtin "empty" template is handled here;
            // every other template is generated by the matching plugin via the resolver.
    ...
    /**
     * Dispatches template scaffolding. The builtin "empty" template is generated directly; every
     * other template is generated by the matching plugin's [ProjectTemplateGenerator] resolved
     * through [templateResolver]. If no plugin contributes the template, nothing is scaffolded.
     */
    private suspend fun generateTemplate(project: AzoraProjectModel, projectPath: String) {
        when (project.template) {
            BUILTIN_TEMPLATE_ID_EMPTY -> EmptyTemplateGenerator(fileSystem).generate(project, projectPath)
