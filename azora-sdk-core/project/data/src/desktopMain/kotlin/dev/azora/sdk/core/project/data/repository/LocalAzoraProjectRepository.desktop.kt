...
actual class LocalAzoraProjectRepository(
    ...
    private val templateResolver: ProjectTemplateGeneratorResolver
    ...
    /**
     * Dispatches template scaffolding. [ProjectTemplate.EMPTY] is builtin; every other template is
     * generated by an installed plugin's [ProjectTemplateGenerator] resolved through
     * [templateResolver]. If no plugin contributes the template, nothing is scaffolded.
     */
    private suspend fun generateTemplate(project: AzoraProjectModel, projectPath: String) {
        when (project.template) {
            ...
            else -> templateResolver.generatorFor(project.template)?.generate(project, projectPath, fileSystem)
        }
    }
